...
namespace SnakeGame
{
    class Program
    {
        ...
        static List<int[]> mines = new List<int[]>(); // Lista de posiciones de las minas
        ...
        static void Main(string[] args)
        {
            ...
            while (!gameOver)
            {
                ...
                if (IsHittingMine()) // Comprobar si la serpiente golpea una mina
                {
                    gameOver = true;
                    break;
                }
        ...
        {
            // Colocar un número aleatorio de minas en posiciones aleatorias
            int numMines = random.Next(5, 10);
            for (int i = 0; i < numMines; i++)
            {
                int[] mine = new int[] { random.Next(1, width), random.Next(1, height) };
                mines.Add(mine);
